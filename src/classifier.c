...
/* Los nombres de los diccionarios no cambian, solo su contenido */
...
/* Tabla hash por diccionario
   Cada diccionario tiene su propia tabla hash, se calcula un numero (hash)
   a partir del texto de la palabra y se va directo al "cajon" donde
   esa palabra deberia estar (busqueda O(1)).
*/
...
/* Funcion hash: recorre la palabra caracter por caracter
...
static void hashInsertar(Diccionario *dic, const char *palabra, int indice){
    ...
    /* Se inserta al inicio de la lista de ese cajon */
